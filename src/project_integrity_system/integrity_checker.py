        ...
        for doc in self.documents.values():
            if isinstance(doc, ProblemStatement):
                ...
                if not any(linked_id.startswith('@RES-') for linked_id in doc.get_linked_ids()):
                    errors.add(f"Validation error in {doc.id}: Problem Statement must have at least one linked research item")
            elif isinstance(doc, Requirement):
                if not any(linked_id.startswith('@PROB-') for linked_id in doc.get_linked_ids()):
                    errors.add(f"Validation error in {doc.id}: Requirement must have at least one linked problem statement")
                if not any(linked_id.startswith('@TEST-') for linked_id in doc.get_linked_ids()):
                    errors.add(f"Validation error in {doc.id}: Requirement must have at least one linked test case")
