...
def test_integrity_checker():
    ...
    errors = checker.validate_all()
    assert len(errors) == 5
    assert any("Requirement @REQ-002 must be linked to at least one Problem Statement" in error for error in errors)
    assert any("Requirement @REQ-002 must have at least one linked test case" in error for error in errors)
